...
// ── Tool definitions (Anthropic format: input_schema, not parameters) ──────────

const TOOLS = [
  {
    name: 'add_entry',
    description: 'Insert a new entry (task, note, decision, meeting, idea, log, analysis, plan, post, file) into the knowledge base.',
    input_schema: {
      type: 'object',
      properties: {
        type: {
          type: 'string',
          enum: ['task', 'note', 'decision', 'meet', 'idea', 'log', 'analysis', 'plan', 'post', 'file'],
          description: 'Entry type',
        },
        title: { type: 'string', description: 'Entry title (required)' },
        content: { type: 'string', description: 'Entry body / details' },
        project: { type: 'string', description: 'Project name to link this entry to' },
  ...
  {
    name: 'get_entries',
    description: 'Retrieve entries from the knowledge base with optional filters.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'get_people',
    description: 'List people in the knowledge base.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'update_person',
    description: 'Update an existing person record by id or name.',
    input_schema: {
      type: 'object',
      properties: {
        id: { type: 'string', description: 'Person UUID' },
        name: { type: 'string', description: 'Person name to look up if id not provided' },
        role: { type: 'string' },
        company: { type: 'string' },
        email: { type: 'string' },
        notes: { type: 'string' },
  ...
  {
    name: 'add_project',
    description: 'Add or upsert a project in the knowledge base.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'get_projects',
    description: 'List projects in the knowledge base.',
    input_schema: {
      type: 'object',
      properties: {
        status: { type: 'string', description: 'Filter by status (active, paused, done)' },
      },
    },
  },
  {
    name: 'get_rules',
    description: 'Retrieve AI behavior rules.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'query_context',
    description: 'Semantic RAG search over all stored knowledge using natural language.',
    input_schema: {
      type: 'object',
      properties: {
        question: { type: 'string', description: 'Natural language question (required)' },
  ...
  {
    name: 'search_memory',
    description: 'Text-based search in entries by title and content.',
    input_schema: {
      type: 'object',
      properties: {
...
async function getEmbedding(
  ...
  try {
    const { data } = await supabase.functions.invoke('embed', { body: { input: text } })
  ...
  switch (name) {
    case 'add_entry': {
      ...
      const { data: entry, error } = await supabase
        .from('entries')
        .insert({ type, title, content: content ?? null, project_id: projectId, tags: tags ?? [], status: status ?? null })
    ...
    case 'get_entries': {
      ...
      let query = supabase
        .from('entries')
        .select('id, type, title, content, status, tags, created_at, projects(name)')
    ...
    case 'get_people': {
      const { company } = input
      let query = supabase.from('people').select('id, name, role, company, email, notes').order('name')
    ...
    case 'update_person': {
      ...
      let resolvedId = id
      if (!resolvedId && name) {
        const { data } = await supabase.from('people').select('id').ilike('name', name).limit(1).single()
        resolvedId = data?.id
      }
      if (!resolvedId) throw new Error('Person not found — provide id or a valid name')
    ...
    case 'add_project': {
      ...
      const { data, error } = await supabase
        .from('projects')
        .upsert({ name, description: description ?? null, company: company ?? null, stack: stack ?? [], status }, { onConflict: 'name' })
    ...
    case 'get_projects': {
      const { status } = input
      let query = supabase.from('projects').select('id, name, company, stack, status, description, created_at').order('name')
      if (status) query = query.eq('status', status)
      const { data, error } = await query
      if (error) throw new Error(error.message)
      return data ?? []
...
// ── OpenRouter call (non-streaming for tool loop) ─────────────────────────────
...
Deno.serve(async (req) => {
  ...
  // ── Input validation ──
  let inputMessages: AnthropicMessage[]
  try {
  ...
  ;(async () => {
    try {
      const conversationMessages: AnthropicMessage[] = [...inputMessages]
      let iteration = 0
      ...
      while (iteration < MAX_ITER) {
        ...
        // Anthropic via OpenRouter wraps the response in choices[0].message for non-streaming
        // but also supports the native Anthropic response shape. Normalize both.
        // deno-lint-ignore no-explicit-any
        const message: any = json.choices?.[0]?.message ?? json

        // content may be a string (no tools) or array of blocks (with tools)
        const rawContent = message.content
        // deno-lint-ignore no-explicit-any
        const contentBlocks: any[] = Array.isArray(rawContent)
          ? rawContent
          : [{ type: 'text', text: typeof rawContent === 'string' ? rawContent : '' }]

        const stopReason: string = json.choices?.[0]?.finish_reason ?? json.stop_reason ?? 'end_turn'

        if (stopReason !== 'tool_use' && !hasToolUse(contentBlocks)) {
          // No tool calls — stream this as the final response
          ...
          // Emit final text as token stream (simulate streaming by chunking)
        ...
        // Has tool_use — execute tools
        // deno-lint-ignore no-explicit-any
        const toolUseBlocks = contentBlocks.filter((b: any) => b.type === 'tool_use')
        const toolResults: { tool_use_id: string; content: string; type: 'tool_result' }[] = []

        for (const block of toolUseBlocks) {
          await writer.write(sseEvent({ tool_call: { id: block.id, name: block.name, params: block.input } }))

          let result: unknown
          let status: 'done' | 'error' = 'done'
          try {
            result = await executeTool(block.name, block.input ?? {}, supabase)
          } catch (err) {
            status = 'error'
          ...
          await writer.write(sseEvent({ tool_result: { id: block.id, status, result: resultStr } }))
      ...
      // ── MAX_ITER reached — do a final non-tool streaming call ──
      const finalRes = await callOpenRouter(apiKey, conversationMessages, true)
      ...
      try {
        await writer.write(sseEvent({ error: err instanceof Error ? err.message : String(err) }))
        await writer.write(sseDone())
