...
const TYPE_COLOR: Record<string, string> = {
  ...
  analysis: '#06B6D4', plan: '#8B5CF6', post: '#F97316', file: '#64748B',
}
...
const PROJECT_STATUS_COLOR: Record<string, string> = {
  active: '#22C55E', paused: '#EAB308', archived: '#71717A', completed: '#3B82F6',
}
...
export function SectionDashboard({ direction, onNavigateTo }: SectionProps) {
  ...
  const [recentProjects, setRecentProjects] = useState<{ id: string; name: string; status: string; description?: string }[]>([])
  ...
  useEffect(() => {
    async function load() {
      ...
      // type distribution
      const { data: typeData } = await supabase.from('entries').select('type')
      if (typeData) {
        const counts: Record<string, number> = {}
        typeData.forEach(r => { counts[r.type] = (counts[r.type] ?? 0) + 1 })
      ...
      const { data: projData } = await supabase
        .from('projects').select('id, name, status, description')
  ...
  return (
    <SectionWrapper direction={direction}>
        ...
        {statItems.map((s, i) => (
          <motion.div
            ...
            style={{
              ...
              originX: 0, cursor: 'pointer', position: 'relative', overflow: 'hidden',
            ...
            animate="visible"
          >
            <div style={{ height: 2, background: s.color, flexShrink: 0 }} />
            <div style={{ padding: '12px 20px 14px', flex: 1, display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              <div style={{ fontFamily: 'var(--font-space-grotesk)', fontSize: 28, fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1, letterSpacing: '-0.02em' }}>
                {s.value}
              </div>
              <div style={{ fontFamily: 'var(--font-inter)', fontSize: 10, color: 'var(--text-muted)' }}>{s.sub}</div>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0, overflow: 'hidden' }}>
              ...
              <motion.div variants={listContainerVariants} initial="initial" animate="animate">
                {logs.map((log, i) => (
                  <motion.div
                    ...
                    style={{
                      display: 'grid', gridTemplateColumns: '40px 68px 74px 1fr',
                    ...
                    whileHover={{ backgroundColor: 'var(--surface-2)' }}
                  >
                    <span style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10, color: 'var(--text-muted)', opacity: 0.6 }}>
                      {log.time}
                    </span>
                    <div style={{
        ...
        {/* COL B — Type breakdown + Projects */}
        <div style={{ width: 240, flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--border)' }}>

          {/* Type distribution */}
          <motion.div
            variants={contentItemVariants}
            style={{ padding: '9px 16px 7px', flexShrink: 0, borderBottom: '1px solid var(--border)' }}
          >
            <span style={{ fontFamily: 'var(--font-inter)', fontSize: 10, color: 'var(--text-secondary)', fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
              By type
            </span>
          ...
          <div style={{ padding: '8px 16px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
            {typeDist.length === 0 ? (
              <span style={{ fontFamily: 'var(--font-inter)', fontSize: 11, color: 'var(--text-muted)' }}>No data</span>
            ) : (
              typeDist.map(({ type, count }) => (
                <div key={type} style={{ marginBottom: 7, cursor: 'pointer' }}
                  onClick={() => onNavigateTo?.(SECTION_INDEX.ENTRIES)}
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                      <div style={{ width: 6, height: 6, borderRadius: 1, background: TYPE_COLOR[type] ?? '#71717A', flexShrink: 0 }} />
                      <span style={{ fontFamily: 'var(--font-inter)', fontSize: 11, color: 'var(--text-secondary)' }}>{type}</span>
                    </div>
                    <span style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10, color: 'var(--text-muted)' }}>{count}</span>
                  </div>
                  <div style={{ height: 3, background: 'var(--surface-3)', borderRadius: 2, overflow: 'hidden' }}>
                    <motion.div
                      ...
                      style={{ height: '100%', background: TYPE_COLOR[type] ?? '#71717A', borderRadius: 2 }}
                    />
                  </div>
                </div>
              ))
          ...
          <motion.div
            variants={contentItemVariants}
            style={{ padding: '9px 16px 8px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}
          >
            <span style={{ fontFamily: 'var(--font-inter)', fontSize: 10, color: 'var(--text-secondary)', fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', display: 'block', marginBottom: 8 }}>
              Status
            </span>
            <div style={{ display: 'flex', height: 5, borderRadius: 3, overflow: 'hidden', gap: 1, marginBottom: 8 }}>
              {statusSegments.map(({ n, c, l }) => n > 0 && (
                <div key={l} title={`${l}: ${n}`} style={{ flex: n / totalStatus, background: c, borderRadius: 2, minWidth: 3 }} />
              ))}
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 12px' }}>
              {statusSegments.map(({ n, c, l }) => (
                <div key={l} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10, color: 'var(--text-muted)' }}>{n} {l}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <div data-inner-scroll style={{ flex: 1, overflowY: 'auto' }}>
            {recentProjects.length === 0 ? (
              <div style={{ padding: '16px', fontFamily: 'var(--font-inter)', fontSize: 11, color: 'var(--text-muted)' }}>No projects yet</div>
            ) : (
              <motion.div variants={listContainerVariants} initial="initial" animate="animate">
                {recentProjects.map(proj => (
                  ...
                  >
                    <div style={{
                      ...
                      background: PROJECT_STATUS_COLOR[proj.status] ?? '#71717A',
                    }} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontFamily: 'var(--font-inter)', fontSize: 12, fontWeight: 500, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {proj.name}
                      </div>
                      {proj.description && (
                        <div style={{ fontFamily: 'var(--font-inter)', fontSize: 10, color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: 1 }}>
                          {proj.description}
                        </div>
                      )}
                    </div>
                    <span style={{
                      fontFamily: 'var(--font-jetbrains-mono)', fontSize: 9,
                      color: PROJECT_STATUS_COLOR[proj.status] ?? 'var(--text-muted)',
                      flexShrink: 0,
                    }}>
                      {proj.status}
                    </span>
        ...
        <div style={{ width: 260, flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
          <motion.div
            variants={contentItemVariants}
            style={{ padding: '9px 16px 8px', flexShrink: 0, borderBottom: '1px solid var(--border)' }}
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontFamily: 'var(--font-inter)', fontSize: 10, color: 'var(--text-secondary)', fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
                Active tasks
              ...
              <span style={{
                fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10, color: 'var(--text-muted)',
          ...
          <div data-inner-scroll style={{ flex: 1, overflowY: 'auto', padding: '8px 10px' }}>
